...
namespace certify {
	...
	void Test_SysDBI_B() {
		...
		ret = sysdbi_b.OpenDB( "I:\\test", "I:\\test\\test.db", 10 * 1024 * 1024, error );
		if( ret != 0 ) {
		...
		ret = sysdbi_b.PutString( "key_test_1", value_test.c_str(), error );
		if( ret != 0 ) {
			FormatLibrary::StandardLibrary::FormatTo( log_info, "存储 字符串 失败！{0} {1}", ret, error );
			syslog->LogWrite( basicx::syslog_level::c_error, log_cate, log_info );
		}
		else {
			ret = sysdbi_b.GetString( "key_test_1", string_test, error );
			if( ret != 0 ) {
		...
		ret = sysdbi_b.PutBinary( "key_test_2", buffer_test, 256, error );
		if( ret != 0 ) {
			FormatLibrary::StandardLibrary::FormatTo( log_info, "存储 二进制 失败！{0} {1}", ret, error );
			syslog->LogWrite( basicx::syslog_level::c_error, log_cate, log_info );
		}
		else {
			ret = sysdbi_b.GetBinary( "key_test_2", binary_test, length_test, error );
			if( ret != 0 ) {
		...
		for( size_t i = 0; i < 5; i++ ) {
			...
			ret = sysdbi_b.PutBinary( string_keys.c_str(), buffer_test, 64, error );
			if( ret != 0 ) {
				FormatLibrary::StandardLibrary::FormatTo( log_info, "存储 测试集 失败！{0} {1}", ret, error );
				syslog->LogWrite( basicx::syslog_level::c_error, log_cate, log_info );
			}
		}
		sysdbi_b.PrintAllRecord();

		ret = sysdbi_b.DelRecord( "keys_1", error );
		if( ret != 0 ) {
			FormatLibrary::StandardLibrary::FormatTo( log_info, "删除 测试项 失败！{0} {1}", ret, error );
			syslog->LogWrite( basicx::syslog_level::c_error, log_cate, log_info );
		}
		ret = sysdbi_b.DelRecord( "keys_3", error );
		if( ret != 0 ) {
			FormatLibrary::StandardLibrary::FormatTo( log_info, "删除 测试项 失败！{0} {1}", ret, error );
			syslog->LogWrite( basicx::syslog_level::c_error, log_cate, log_info );
		}
		...
		ret = sysdbi_b.DelAllRecord( error );
		if( ret != 0 ) {
